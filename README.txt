...
(API GURARD) INSTALL 3rd Libraries
——————————————
1) install composer

2) install lib defuse/php-encryption (under project root [apigurad])
./composer.phar require defuse/php-encryption
./composer.phar require nategood/httpful

Optional (if got ssl error when running composer update):

1) find default_cert_file location
...
3) sudo mv cacert.pem /opt/lampp/share/openssl/cert.pem
